    ...
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        ...
        <div class="row">
                ...
                <div class="card card-primary card-outline text-sm">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <?php if (isset($config['product'][$type]['check_item'])) {
                                foreach ($config['product'][$type]['check_item'] as $key => $value) { ?>
                                ...
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    ...
                                    <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea
                                                class="form-control for-seo text-sm <?= (isset($config['product'][$type]['desc_cke_item']) && $config['product'][$type]['desc_cke_item'] == true) ? 'form-control-ckeditor' : '' ?>"
                                                name="data[desc<?= $k ?>]" id="desc<?= $k ?>" rows="5"
                                                placeholder="Mô tả (<?= $k ?>)"><?= htmlspecialchars_decode((!empty($flash->has('desc' . $k))) ? $flash->get('desc' . $k) : @$item['desc' . $k]) ?></textarea>
                                        </div>
                                        <?php } ?>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea
                                                class="form-control for-seo text-sm <?= (isset($config['product'][$type]['content_cke_item']) && $config['product'][$type]['content_cke_item'] == true) ? 'form-control-ckeditor' : '' ?>"
                                                name="data[content<?= $k ?>]" id="content<?= $k ?>" rows="5"
                                                placeholder="Nội dung (<?= $k ?>)"><?= htmlspecialchars_decode((!empty($flash->has('content' . $k))) ? $flash->get('content' . $k) : @$item['content' . $k]) ?></textarea>
                                        </div>
                                        <?php } ?>
                                    </div>
                                    <?php } ?>
            ...
            <div class="col-xl-4">
                ...
                <!-- <?php if (isset($config['product'][$type]['images_item2']) && $config['product'][$type]['images_item2'] == true) { ?>
                <div class="card card-primary card-outline text-sm">
                    <div class="card-header">
                        <h3 class="card-title">Hình ảnh 2 <?= $config['product'][$type]['title_main_item'] ?></h3>
                        <div class="card-tools">
                            <button type="button" class="btn btn-tool" data-card-widget="collapse"><i
                                    class="fas fa-minus"></i></button>
                        </div>
                    ...
                    <div class="card-body">
                        <?php
                            ...
                            $photoDetail['dimension'] = "Width2: " . $config['product'][$type]['width_item2'] . " px - Height2: " . $config['product'][$type]['height_item2'] . " px (" . $config['product'][$type]['img_type_item'] . ")";
                            ?>
